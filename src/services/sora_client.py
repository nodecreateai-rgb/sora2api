...
class SoraClient:
    ...
    async def _get_oai_did_via_session(
        ...
        def _do_request() -> Tuple[Optional[str], str]:
            scraper = cloudscraper.create_scraper()
            if proxy_url:
                scraper.proxies = {"http": proxy_url, "https": proxy_url}
            if cookie_header:
            ...
            cookie_dump = "; ".join([f"{c.name}={c.value}" for c in scraper.cookies])
            return scraper.cookies.get("oai-did"), cookie_dump
        ...
        try:
            ...
            if "400" in error_str or "sentinel" in error_str.lower() or "invalid" in error_str.lower():
                ...
                if scraper_result:
                    ...
                    scraper_token, scraper_ua, scraper_cookie, scraper_device_id = scraper_result
                    scraper_data = json.loads(scraper_token)
                    scraper_device_id = scraper_data.get("id", scraper_device_id)
    ...
    async def generate_video(self, prompt: str, token: str, orientation: str = "landscape",
        ...
        if not scraper_result:
            ...
            sentinel_token, user_agent, cookie_header, device_id = scraper_result
        
        result = await self._nf_create_urllib(
            token, json_data, sentinel_token, proxy_url, token_id, user_agent, cookie_header, device_id
        )
        return result["id"]
